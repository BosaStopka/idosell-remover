...
def worker():
    while True:
        ...
        # Logika wyboru w queueing.next_job (czysta, testowalna bez app.py).
        job_id = queueing.next_job(priority_queue, queue, process_paused.is_set())
        if job_id is None:
            if process_paused.is_set():
                time.sleep(0.5)           # bulk wstrzymany - nie busy-spinuj
            continue
...
# przeniesione do gallery.py (czysta funkcja, testowalna bez importu app.py);
# alias zachowuje dotychczasowe wywolania _gallery_ordered(...) w tym pliku.
_gallery_ordered = gallery.gallery_ordered
